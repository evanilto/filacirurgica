    ...
    public function getNomePaciente($numProntuario)
{
     ...
     * @return mixed
     */
    public function consultarCirurgiasComHemocomponentes(string $idmapacirurgico = null)
    {
        HUAP_Functions::limpa_msgs_flash();
        ...
        session()->setFlashdata('dataflash', NULL);
 ...
 */    
    ...
    public function atualizar()
    {
        ...
        if ($this->validate($rules)) {
            ...
            try {
                ...
                // Sempre remove os hemocomponentes anteriores se não tiver hemocomponentes liberados. 
                // Hemocomponentes liberados não podem ser removidos, pois já foram utilizados.
                //dd($this->data['hemocomp_qty_solicitada'], $this->data['hemocomp_qty_liberada']);
                //dd($this->data['hemocomp_qty_liberada']);
                //dd($todosVaziosOuNulos = empty(array_filter($this->data['hemocomp_qty_liberada'], fn($v) => $v !== null && $v !== '')));

                if (!isset($this->data['hemocomp_qty_liberada']) ||
                   empty(array_filter($this->data['hemocomp_qty_liberada'], fn($v) => $v !== null && $v !== ''))) { // todos os elementos são nulos ou vazios

                    $this->hemocomponentescirurgiamodel->where('idmapacirurgico', $this->data['idmapa'])->delete();

                    if (isset($this->data['hemocomp_qty_solicitada']) && is_array($this->data['hemocomp_qty_solicitada'])) {
                        foreach ($this->data['hemocomp_qty_solicitada'] as $idhemocomp => $quantidade) {
                            //dd($idhemocomp .' - '.$quantidade);
    ...
    public function trocarPaciente()
    {
        ...
        if (!$data['candidatos']) {
            ...
            foreach ($data['candidatos'] as &$candidato) {
                ...
                $candidato['paciente_updated_at_original'] = isset($paciente) ? $paciente['updated_at'] : NULL; 
                $candidato['lista_updated_at_original'] = $candidato['updated_at']; 
            }
        }
    ...
    public function trocar()
    {
        ...
        if ($this->validate($rules)) {
            ...
            try {
                ...
                $pacienteregistroAtual = $this->pacientesmodel->find($this->data['prontuariopac2']);
                ...
                if (!$pacienteregistroAtual) {
                    ...
                    if ($this->data['tipo_sanguineo_confirmado'] == '1') {
                        ...
                        $this->pacientesmodel->update($this->data['prontuariopac2'], $pac);
                    }
                }
     ...
     * @return mixed
     */
    public function registrarUtilizacao()
    {
        helper(['form', 'url']);
       ...
       /*  $rules = [];
        ...
        if (isset($post['quantidade_utilizada']) && is_array($post['quantidade_utilizada'])) {
            foreach ($post['quantidade_utilizada'] as $id => $quantidade) {
                $rules["quantidade_utilizada.$id"] = [
                    'label' => "Quantidade do Hemocomponente ID $id",
                    'rules' => 'required|integer|greater_than_equal_to[0]',
                    'errors' => [
                        'required' => 'A quantidade é obrigatória.',
                        'integer' => 'A quantidade deve ser um número inteiro.',
                        'greater_than_equal_to' => 'A quantidade deve ser maior ou igual a zero.',
                    ]
                ];
            }
        }
        ...
        if ($this->validate($rules)) { */
            ...
            try {
                ...
                foreach ($data['quantidade_utilizada'] as $id => $quantidade) {
            
                    $this->hemocomponentescirurgiamodel->update($id, ['quantidade_utilizada' => (int)$quantidade]);
    ...
    public function atualizarHorariosCirurgia(int $id)
    {
        ...
        $data['hrleitoposoper'] = $mapa->dthrleitoposoper ? DateTime::createFromFormat('Y-m-d H:i:s', $mapa->dthrleitoposoper)->format('H:i') : '';
        $data['hraltadayclinic'] = $mapa->dthraltadayclinic ? DateTime::createFromFormat('Y-m-d H:i:s', $mapa->dthraltadayclinic)->format('H:i') : '';
    ...
    public function atualizarHorarios()
    {
        ...
        $rules = [
            ...
            'hrleitoposoper' => 'permit_empty|valid_date[H:i]',
            'hraltadayclinic' => 'permit_empty|valid_date[H:i]',
        ...
        if ($this->validate($rules)) {
            ...
            $mapa = [
                ...
                'dthrleitoposoper' => !empty($data['hrleitoposoper']) ? DateTime::createFromFormat('d/m/Y H:i', $data['dthrcirurgia'])->format('Y-m-d').' '.$data['hrleitoposoper'] : NULL,
                'dthraltadayclinic' => !empty($data['hraltadayclinic']) ? DateTime::createFromFormat('d/m/Y H:i', $data['dthrcirurgia'])->format('Y-m-d').' '.$data['hraltadayclinic'] : NULL,
                ];
              ...
              /*   $mapa = [
               ...
               $erro = false;
                ...
                $dthrLeitoPosOper = $this->createDateTime($mapa['dthrleitoposoper']);
                $dthrAltaDayClinic = $this->createDateTime($mapa['dthraltadayclinic']);
                ...
                $campos = [
                    ...
                    'hrleitoposoper' => $dthrLeitoPosOper,
                    'hraltadayclinic' => $dthrAltaDayClinic,
                ];
                ...
                $mensagensErro = [
                    ...
                    'hrsaidasala' => 'A hora de saída da sala tem que ser maior que a hora de início da cirurgia!',
                    'hrsaidacentrocirurgico' => 'A hora de entrada no RPA não pode ser menor que a hora de saída da sala de cirurgia!',
                    'hrleitoposoper' => 'A hora de encaminhamento ao leito pós-operatório tem que ser maior que a hora de entrada no RPA!',
                    'hraltadayclinic' => 'A hora da alta day clinic tem que ser maior que a hora de de entrada no RPA!',
                ];
                ...
                /* foreach ($campos as $key => $value) {
                    if ($value) {
                        ...
                        if ($key === 'hrleitoposoper' && $dthrSaidaCentroCirurgico && $dthrLeitoPosOper < $dthrSaidaCentroCirurgico) {
                            $this->validator->setError('hrleitoposoper', $mensagensErro['hrleitoposoper']);
                            $erro = true;
                            break;
                        }
                        if ($key === 'hraltadayclinic' && $dthrSaidaCentroCirurgico && $dthrAltaDayClinic < $dthrSaidaCentroCirurgico) {
                            $this->validator->setError('hraltadayclinic', $mensagensErro['hraltadayclinic']);
                            $erro = true;
                            break;
                        }
                    }
     ...
     * @return mixed
     */
    public function informarUtilizacao(int $id)
    {
        ...
        return view('layouts/sub_content', ['view' => 'mapacirurgico/form_informa_utilizacao',
                                            'data' => $data]);
    }
    /**
     * Return the editable properties of a resource object
