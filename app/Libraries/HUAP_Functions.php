...
class HUAP_Functions
{
    ...
    */
    ...
    /**
    * Função que verifica se os menus que o usuário pode acessar
    *
    *
    * @return boolean
    */
    public static function tem_permissao($menu) {
       ...
       //die(var_dump($_SESSION['Sessao']['idPerfil']));
        ...
        switch ($menu) {
            ...
            case 'transfusao-requisitar':
                return !empty(array_intersect($_SESSION['Sessao']['idPerfil'], ['1','71']));

            case 'transfusao-atender':
                return !empty(array_intersect($_SESSION['Sessao']['idPerfil'], ['1','72']));
    ...
    * @return mixed
    */
    public static function limpa_msgs_flash() {

        helper(['form', 'session']);

        session()->setFlashdata('success', null);
        session()->setFlashdata('failed', null);
        session()->setFlashdata('warning_message', null);
        session()->setFlashdata('nochange', null);
    }
    /**
    * 
    *
    ...
    * @return mixed
    */
    public static function DataValida($data, $formato) {
        try {
            $dateTimeObj = DateTime::createFromFormat($formato, $data);
    
            if ($dateTimeObj) {
                // A criação do objeto DateTime foi bem-sucedida
                $dataFormatada = $dateTimeObj->format($formato);
                ...
                // A criação do objeto DateTime falhou
                return false;
            }
        } catch (\Exception $e) {
            // Registre a exceção ou imprima para debug, se necessário
            // error_log($e->getMessage());
    
            return false; // A data não é válida devido a uma exceção
        }
    }
